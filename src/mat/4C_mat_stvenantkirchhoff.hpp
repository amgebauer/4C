...
namespace Mat
{
  ...
  class StVenantKirchhoff : public So3Material
  {
   public:
    ...
    static constexpr Core::LinAlg::SymmetricTensor<double, 3, 3, 3, 3>
    evaluate_stress_linearization(const double E, const double nu)
    {
      const double mue = E / (2 * (1 + nu));
      const double lambda = E * nu / ((1 + nu) * (1 - 2 * nu));
      return 2 * mue * Core::LinAlg::TensorGenerators::symmetric_identity<double, 3, 3, 3, 3> +
             lambda * Core::LinAlg::dyadic(Core::LinAlg::TensorGenerators::identity<double, 3, 3>,
                          Core::LinAlg::TensorGenerators::identity<double, 3, 3>);
    }
