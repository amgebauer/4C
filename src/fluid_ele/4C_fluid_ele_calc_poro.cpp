  ...
  if (Base::is_higher_order_ele_)
  {
    ...
    if (kintype_ == Inpar::Solid::KinemType::nonlinearTotLag)
    {
      ...
      if (nsd_ == 3)
      {
        ...
        glstrain(2, 2) = 0.5 * (cauchygreen(2, 2) - 1.0);
        glstrain(0, 1) = 0.5 * cauchygreen(0, 1);
        glstrain(1, 2) = 0.5 * cauchygreen(1, 2);
        glstrain(2, 0) = 0.5 * cauchygreen(2, 0);
      }
      else if (nsd_ == 2)
      {
        glstrain(0, 0) = 0.5 * (cauchygreen(0, 0) - 1.0);
    ...
    static Core::LinAlg::Matrix<nsd_, nsd_> stress_mat(Core::LinAlg::Initialization::uninitialized);
    if (nsd_ == 3)
    {
      stress_mat(0, 0) = stress(0, 0);
      stress_mat(0, 1) = stress(0, 1);
      stress_mat(0, 2) = stress(0, 2);
      stress_mat(1, 0) = stress(1, 0);
      stress_mat(1, 1) = stress(1, 1);
      stress_mat(1, 2) = stress(1, 2);
      stress_mat(2, 0) = stress(2, 0);
      stress_mat(2, 1) = stress(2, 1);
      stress_mat(2, 2) = stress(2, 2);
    }
    else if (nsd_ == 2)
    {
      stress_mat(0, 0) = stress(0, 0);
    ...
    for (int i = 0; i < nsd_; ++i)
      for (int j = 0; j < nsd_; ++j)
        for (int k = 0; k < nsd_; ++k) mixres_(i) -= F_X(i * nsd_ + j, k) * stress_mat(j, k);
